...
### No custom user ID for Stripe paywall

Paywalls with Stripe products (like Apple Pay via Stripe) require a custom user ID to be set before showing. Otherwise, a fallback will be shown.

To fix this, set a custom user ID before triggering the paywall:

```swift
Helium.identify.userId = "your-custom-user-id"
```

Stripe uses this ID to associate the purchase with your user's account.
