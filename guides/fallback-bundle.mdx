...
### Fallback banner (iOS, debug builds)
...
<Note>
  The banner reads "Fallback Paywall" with a "Tap for details" subtitle, an info icon, and a dismiss (X) button. It only appears in iOS debug builds — it never shows in TestFlight or App Store builds, so your users will never see it. Tapping the banner opens Helium's diagnostic view explaining why the fallback was shown. Because the full-width banner overlays the top corners where a paywall's own close button usually sits, tap the X to dismiss the banner first so you can reach the paywall's close button. Opening diagnostics respects the pre-existing `Helium.config.paywallNotShownDiagnosticDisplayEnabled` setting and a persistent "don't show again" choice — if diagnostics are disabled, tapping does nothing.
</Note>
