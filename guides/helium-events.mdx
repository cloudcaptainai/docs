...
<Tabs>
  ...
  <Tab title="React Native">
    ...
    export type HeliumPaywallEvent = {
      ...
      paywallName?: string;
      productId?: string;
      origin?: 'restorePurchases' | 'duringPurchase' | 'detected';
      buttonName?: string;
      configId?: string;
      numAttempts?: number;
...
| **purchaseRestored**       | Fires when an existing entitlement is surfaced via restore or purchase flow                                             | `productId` (String) - Restored product ID<br />`triggerName` (String) - Associated trigger<br />`paywallName` (String) - Paywall name<br />`isSecondTry` (Boolean) - True if paywall is "second try" flow<br />`origin` (String) - How the restore was surfaced: `"restorePurchases"` (user tapped Restore Purchases button), `"duringPurchase"` (purchase action resolved as restoration), or `"detected"` (entitlement passively observed after external web checkout)<br />`timestamp` (Number) - When event occurred |
