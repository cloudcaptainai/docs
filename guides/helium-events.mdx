...
### PaywallEntitledEvent (onEntitled)

`PaywallEntitledEvent` is not a separate event — it's the entitling event passed to the `onEntitled` callback of `presentUpsell`, narrowed to the events that can entitle a user:
...
- `paywallSkipped` — the paywall was not shown because the user is already entitled (requires `dontShowIfAlreadyEntitled`)

For `purchaseSucceeded`, `purchaseRestored`, and `purchaseAlreadyEntitled`, `onEntitled` fires when the paywall closes. If `dontShowIfAlreadyEntitled` is `true` and the user is already entitled, the paywall is skipped and `onEntitled` fires immediately with a `paywallSkipped` event — skips due to targeting or workflow configuration do not call `onEntitled`. Each event carries the fields listed for its type in the table above (e.g. `productId` on the purchase events, but only `triggerName` and `timestamp` on `paywallSkipped`).
  ...
  onEntitled: (event) => {
    switch (event?.type) {
      ...
      case 'purchaseAlreadyEntitled':
        // purchase attempt resolved to an existing entitlement (iOS only)
        break;
      case 'paywallSkipped':
        // paywall not shown - user already entitled
        break;
    }
  },
});
...
The event argument is optional, so existing no-argument `onEntitled` handlers keep working unchanged.
