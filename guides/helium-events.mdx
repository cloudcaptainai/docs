...
| **paywallSkipped**         | Fires if paywall display is skipped due to a targeting holdout or existing entitlement. See [PaywallSkippedEvent](#paywallskippedevent) for the `skipReason` delivered to the `onPaywallSkip` callback.                   | `triggerName` (String) - Associated trigger<br />`timestamp` (Number) - When event occurred                                                                                                                                                                                                                                                                                                                                                                                                     |
...
| **purchaseAlreadyEntitled** | Fires when a purchase attempt resolves to an entitlement the user already had (iOS only). Not a new revenue event                | `productId` (String) - Product the user already owns<br />`triggerName` (String) - Associated trigger<br />`paywallName` (String) - Paywall name<br />`isSecondTry` (Boolean) - True if paywall is "second try" flow<br />`storeKitTransactionId` (String, optional) - Existing transaction ID<br />`storeKitOriginalTransactionId` (String, optional) - Original transaction ID<br />`timestamp` (Number) - When event occurred |
...
`PaywallEntitledEvent` is the entitling event delivered to the `onEntitled` callback of `presentUpsell` in the React Native SDK (Expo or bare; see the [React Native quickstart](/sdk/quickstart-react-native)).

It is a narrowed alias of the `HeliumPaywallEvent` type shown in the React Native tab of the Event Types section above. It carries the full `HeliumPaywallEvent` property set (`productId`, `triggerName`, `paywallName`, `isSecondTry`, `timestamp`, etc.), with only `type` restricted to the four entitling values.
...
| `type`                     | Meaning                                                                                                            |
| -------------------------- | ------------------------------------------------------------------------------------------------------------------ |
| `purchaseSucceeded`        | A new purchase completed the entitlement                                                                           |
| `purchaseRestored`         | Entitlement gained via restore                                                                                     |
| `purchaseAlreadyEntitled`  | Purchase attempt resolved to an existing entitlement (**iOS only**)                                                |
| `paywallSkipped`           | Paywall not shown because the user was already entitled (only when `dontShowIfAlreadyEntitled` is `true`)          |

<Note>
  For the purchase, restore, and already-entitled cases the event fires when the paywall closes. For the skip case it fires immediately when the paywall is not shown.
</Note>

Exceptions thrown inside the `onEntitled` callback are caught and logged (`[Helium] onEntitled callback failed`) rather than propagating.

### PaywallSkippedEvent

`PaywallSkippedEvent` is the event delivered to the `onPaywallSkip` callback of `presentUpsell` in the React Native SDK (Expo or bare; see the [React Native quickstart](/sdk/quickstart-react-native)) and the [Flutter SDK](/sdk/quickstart-flutter). It fires immediately when a paywall is intentionally not shown for a trigger.
...
<Tabs>
  ...
  <Tab title="Flutter">
    `PaywallSkippedEvent` carries:

    - `type` — String constant `'paywallSkipped'`
    - `triggerName` — String
    - `skipReason` — a `PaywallSkippedReason` enum with values `targetingHoldout`, `alreadyEntitled`, and `unknown`
  </Tab>
</Tabs>

| `skipReason`       | Meaning                                                                                                   |
| ------------------ | --------------------------------------------------------------------------------------------------------- |
| `targetingHoldout` | The trigger fell into a targeting holdout configured in the Helium workflow.                              |
| `alreadyEntitled`  | The user already had an active entitlement and `dontShowIfAlreadyEntitled` was set to `true`.             |
| `unknown`          | A defensive default that should not occur in normal use.                                                  |

<Note>
  - `targetingHoldout` always routes to `onPaywallSkip`.
  - `alreadyEntitled` routes to `onEntitled` when that handler is provided (React Native only — see [PaywallEntitledEvent](#paywallentitledevent)), and otherwise routes to `onPaywallSkip`.
  - Errors never fire `onPaywallSkip`. In React Native, a paywall that fails to open — and whose fallback also fails — routes to `onPaywallUnavailable` instead. In Flutter, configure a `fallbackPaywall` widget (or `fallbackBundleAssetPath`) at `initialize`, and observe an open failure through the global event listener as a `paywallOpenFailed` event. See the [paywallOpenFailed](#system-events) row and [Fallback Paywalls](/guides/fallback-bundle).
</Note>
