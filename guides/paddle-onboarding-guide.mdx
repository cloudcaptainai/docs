...
<Tabs>
  ...
  <Tab title="React Native / Expo">
    ...
    **Web Apple Pay readiness (optional).** Paddle web checkout offers Apple Pay as its only payment method, so a user with no usable Apple Pay card saved in their device's Wallet reaches the checkout unable to pay. Enable readiness to have the SDK measure whether the checkout browser can complete an Apple Pay payment and report that signal to Helium. Call `setEnableWebApplePayReadiness(true)` before `initialize`:

    ```tsx
    import { setEnableWebApplePayReadiness, initialize } from 'expo-helium';

    setEnableWebApplePayReadiness(true);
    await initialize({ apiKey /* your existing Helium API key */ });
    ```

    - **Off by default.** The setter defaults to `false`. While off, the SDK runs no measurement and adds no launch delay.
    - **Order matters.** The measurement runs inside the prefetch that `initialize` starts, so call the setter before `initialize` — a call made afterward misses the launch it would measure.
    - **iOS only.** The measurement runs on iOS. On Android the call is a safe no-op, so shared React Native / Expo code can ship unchanged.
    - **First launch is delayed.** When enabled, the first launch after install waits up to about 2 seconds so the measurement can be sent. Later launches send the stored value immediately and re-measure in the background. Paywall presentation never blocks on the measurement.

    The setter only reports the signal. After enabling it, configure a [dashboard Workflow](https://app.tryhelium.com/workflows) to act on the signal, for example routing users reported as not ready to an in-app purchase paywall.

    Available in `expo-helium` 3.6.1\+ (which pins the native Helium iOS SDK 4.11.0\+, the release that adds the underlying readiness measurement). On the older `@tryheliumai/paywall-sdk-react-native` package, confirm your installed SDK exposes the setter before relying on it.
  </Tab>
