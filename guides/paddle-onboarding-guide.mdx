...
6. **Failed payment.** Attempt a purchase with a declining test card in Paddle's sandbox and confirm you're deep-linked back with no entitlement granted and no success state shown.
7. **Webhook delivery.** In the Paddle dashboard, confirm webhooks are reaching Helium with 2xx responses.
8. **Pre-login purchase (if applicable).** If your App2Web paywall can be shown before you set a durable `user_id`, make a purchase before setting it. Set the durable user ID afterwards and confirm the entitlement attaches correctly.
...
Deep links are specific to your app build, so it's cleaner to configure them alongside SDK init than in the dashboard. The URL is global: set once, applied to every Paddle purchase. On iOS (helium-swift releases newer than 4.6.0) a single `redirectURL` covers success, cancel, and failure. Earlier iOS versions, React Native, and Flutter take separate `successURL`/`cancelURL` values — it's ok if they are the same URL.
