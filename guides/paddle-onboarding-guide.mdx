...
<Tabs>
  <Tab title="iOS (native, Helium 4.6.0+)">
    ...
    **Add a "Manage Subscription" button** so users can cancel or update payment methods through Paddle's customer portal. Call `getPaddleCustomerId()` to get the current user's Paddle customer ID, then send it to your own server. Your server calls Paddle's [create customer portal session](https://developer.paddle.com/api-reference/customer-portals/create-customer-portal-session) endpoint and returns the portal URL for your app to open.

    `getPaddleCustomerId()` returns the cached ID immediately when it's available, and returns `nil` when the user has no Paddle customer ID or Paddle web checkout isn't configured.
    ...
    guard let customerId = await Helium.shared.getPaddleCustomerId() else {
        print("No Paddle customer ID available")
        return
    }
    
    // Send customerId to your server, which creates a Paddle customer portal
    // session and returns the portal URL, then open it:
    let url = try await fetchPortalURL(forCustomerId: customerId)
    UIApplication.shared.open(url)
    ```

    <Note>
    The SDK's `createPaddlePortalSession()` method is deprecated. Generate portal sessions on your own server from the customer ID instead, so your portal configuration and Paddle API key stay server-side.
    </Note>
  </Tab>
  <Tab title="React Native / Expo">
    ...
    **Add a "Manage Subscription" button** so users can cancel or update payment methods through Paddle's customer portal. Call `getPaddleCustomerId()` to get the current user's Paddle customer ID, then send it to your own server. Your server calls Paddle's [create customer portal session](https://developer.paddle.com/api-reference/customer-portals/create-customer-portal-session) endpoint and returns the portal URL for your app to open.
    ...
    `getPaddleCustomerId()` is iOS only. It returns `undefined` when the user has no Paddle customer ID assigned, or when it's called on any other platform.
    ...
    if (!customerId) {
      console.log('No Paddle customer ID available');
      return;
    }
    
    // Send customerId to your server, which creates a Paddle customer portal
    // session and returns the portal URL, then open it:
    const url = await fetchPortalURL(customerId);
  ...
  <Tab title="Flutter (Helium 3.3.8+)">
    ...
    **Add a "Manage Subscription" button** so users can cancel or update payment methods through Paddle's customer portal. Call `getPaddleCustomerId()` to get the current user's Paddle customer ID, then send it to your own server. Your server calls Paddle's [create customer portal session](https://developer.paddle.com/api-reference/customer-portals/create-customer-portal-session) endpoint and returns the portal URL for your app to open. `launchUrl` comes from the [url_launcher](https://pub.dev/packages/url_launcher) package:

    `getPaddleCustomerId()` is iOS only. It returns `null` when the user has no Paddle customer ID assigned, or when it's called on any other platform.
    ...
    // when the button is tapped:
    final customerId = await HeliumFlutter().getPaddleCustomerId();
    if (customerId == null) {
      log('No Paddle customer ID available');
      return;
    }
    
    // Send customerId to your server, which creates a Paddle customer portal
    // session and returns the portal URL, then open it:
    final url = await fetchPortalURL(customerId);
