...
<Tabs>
  ...
  <Tab title="Flutter (Helium 3.3.8+)">
    ...
    **Handle the post-purchase return.** When checkout completes, the user is deep-linked back to your app via the success URL. Forward incoming links to Helium with `handleURL(url)` — it's safe to forward every link, since URLs that aren't Helium's are ignored. If you don't already handle deep links, add [app_links](https://pub.dev/packages/app_links) with `flutter pub add app_links`:
