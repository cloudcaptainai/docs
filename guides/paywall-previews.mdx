...
- **Simulated purchase** — the checkout looks and behaves like the real flow, but no transaction is created, no revenue events fire, and no entitlement is granted. This is the safest way to verify the experience end to end.
...
- **Configure before each preview** — off by default. When on, the configuration screen appears before each App2Web preview so you can confirm the settings for that launch; when off, previews launch directly with the session settings.
...
### What previews don't cover

Previews bypass audience targeting and do not exercise paywall traits or your post-purchase handlers. Before launch, also open the flow through a real trigger on a physical device — see the [App2Web go-live checklist](/guides/app2web-go-live-checklist).
