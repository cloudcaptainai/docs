...
On Flutter, the triple-tap gesture is enabled by default in dev builds. To turn it off, call `setPaywallPreviewsEnabledInDevBuilds(false)` before showing a paywall.

```dart
HeliumFlutter().setPaywallPreviewsEnabledInDevBuilds(false);
```
