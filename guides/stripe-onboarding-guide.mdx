  ...
  <Tab title="Expo (Helium 3.4.13+)">
    **Enable external web checkout before initializing Helium:**
    ...
    enableExternalWebCheckout({
      redirectURL: "yourapp://openapp",
      paymentProcessors: ["stripe"], // required
    });
    ```

    The `redirectURL` is set once, globally, and covers success, cancel, and payment failure. Make sure it's registered as a deep link in your app (via scheme or universal link).
    ...
    **Handle the post-purchase return.** When checkout finishes — whether the purchase succeeds, is canceled, or fails — the user is deep-linked back to your app via the redirect URL. For a smoother handoff, call `heliumHandleURL(url)` from your deep link handling:
