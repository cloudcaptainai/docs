...
<Tabs>
  <Tab title="iOS (native) 4.6.0+">
    **Enable external web checkout before initializing Helium:**
    ...
    The `redirectURL` is set once, globally. Make sure it is registered as a deep link in your app (via scheme or universal link).
    ...
    **Handle the post-purchase return.** When checkout completes, the user is deep-linked back to your app via the redirect URL. For a smoother handoff, call `Helium.shared.handleURL(url)` from your `onOpenURL` / `application(_:open:)` handler.
