...
<Tabs>
  <Tab title="iOS (native, Helium 4.6.0+)">
    **Enable external web checkout before initializing Helium:**
    ...
    <Note>
      `redirectURL` is not in a released SDK version yet — it ships in the next helium-swift release after 4.6.0. Until then, use the equivalent two-URL form with the same URL for both: `enableExternalWebCheckout(successURL:cancelURL:paymentProcessors:)`. That form will keep working, but is deprecated going forward.
    </Note>
