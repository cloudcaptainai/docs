...
By default, the SDK opens a paywall's navigate-action `http`/`https` links in the external browser. The **`Helium.config.openPaywallLinksInApp`** Bool (default `false`) controls this. Set it to `true` before calling `Helium.shared.initialize(...)` to open those links in an in-app Safari view that slides up over the paywall as a page sheet, keeping the user in your app:

```swift
// Open paywall links in an in-app Safari page sheet over the paywall
