...
For more advanced use cases, such as handling results from the paywall, you can create an `Intent` and launch it manually. This is useful when you need to know if a purchase was successful.

The first step is to create an `Intent` using the `createPaywallIntent` helper function. This function configures and returns an `Intent` that can be used to launch the `HeliumPaywallActivity`.
...
We provide two different ways of presenting the Paywall with an intent:
