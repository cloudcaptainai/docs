...
Helium.shared.presentPaywall(
    ...
    onEntitled: { entitledEvent in
        // How the user became entitled: .purchased, .restored,
        // .alreadyEntitled, or .skipped (requires dontShowIfAlreadyEntitled)
    }
    ...
    case .alreadyEntitled:
        // Only called when onEntitled is not provided;
        // here the .skipped case of onEntitled handles it
        break
    default:
...
<Note>
  `onEntitled` now receives a `PaywallEntitledEvent` (`.purchased`, `.restored`, `.alreadyEntitled`, or `.skipped`) so you can tell how the user became entitled. Existing no-argument closures still compile against a deprecated overload — no code changes are forced.
</Note>
