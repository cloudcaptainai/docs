...
| Event Name | Description | Parameters |
|------------|-------------|------------|
| **userAllocated** | Fires once per trigger when a user is allocated to an experiment variant | `type` (String) - "userAllocated"<br/>`experimentInfo` (Object) - Complete experiment allocation data (see below)<br/>`timestamp` (Number) - When allocation occurred |
