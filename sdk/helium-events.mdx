...
You can also fetch all experiments for the current user with a few methods available on the `Helium` top level object. Make sure to call tehse methods after `Helium.initialize()`

```swift
// Gets all experiments for which this user is eligible
func getAllEnrolledExperiments() -> [ExperimentInfo]?

// Gets all experiments for which this user is eligible AND has already been enrolled
func getActiveEnrolledExperiments() -> [ExperimentInfo]?

// Gets all experiments for which this user is eligible but has NOT been enrolled (e.g., its on a trigger that the user hasn't come across yet)
func getPredictedEnrolledExperiments() -> [ExperimentInfo]?
```
