...
event.triggerName     // Associated trigger
event.paywallName     // Template name
event.transactionId         // StoreKit transaction ID (optional)
event.originalTransactionId // StoreKit original transaction ID (optional)
event.timestamp       // When event occurred
```

<Note>
  The `transactionId` and `originalTransactionId` fields provide StoreKit transaction identifiers for enhanced tracking and analytics. These fields are optional and may be `nil` in some scenarios depending on the delegate implementation and timing constraints.
</Note>
...
func onHeliumPaywallEvent(event: HeliumPaywallEvent) {
    ...
    case .subscriptionSucceeded(let productKey, let triggerName, let paywallTemplateName, let transactionId, let originalTransactionId):
        ...
        if let originalTxnId = originalTransactionId {
            print("Original Transaction ID: \(originalTxnId)")
        }
...
<Note>
  Starting with SDK version 3.0.4, this event now includes optional `transactionId` and `originalTransactionId` parameters that provide StoreKit transaction identifiers for enhanced tracking and analytics.
</Note>
