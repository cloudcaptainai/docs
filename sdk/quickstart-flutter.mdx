...
#### Handling Skipped Paywalls

`onPaywallSkip` fires when a paywall is intentionally not shown for the trigger (for example, a targeting holdout, or an already-entitled user). Add the callback to your `presentUpsell` call:
...
HeliumFlutter().presentUpsell(
  ...
  context: context,
  onPaywallSkip: (event) {
    log('Paywall skipped: ${event.skipReason.name}');
  },
);
```

`onPaywallSkip` is a top-level `presentUpsell` parameter passed alongside `eventHandlers` (not nested inside it), so you can use both in the same call.

See [PaywallSkippedEvent](/guides/helium-events#paywallskippedevent) for the `skipReason` values.
