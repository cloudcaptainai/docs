...
<Note>
  To clear a previously set custom user id (for example, when a user signs out), call `clearCustomUserId()`. This reverts Helium to its auto-generated anonymous user id.

  ```dart
  await heliumFlutter.clearCustomUserId();
  ```

  `resetHelium()` does **not** clear the custom user id, so use `clearCustomUserId()` when you need to unset it.
</Note>
