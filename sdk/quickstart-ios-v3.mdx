...
When displaying a paywall you can pass in event handlers to listen for relevant [Helium Events](/guides/helium-events). You can chain a subset of handlers with builder syntax:
...
<Tabs>
  ...
  <Tab title="Custom Delegate">
    ...
    Visit [Helium Events](/guides/helium-events) for details on the different Helium paywall events.
...
[Helium Events](/guides/helium-events) are emitted by Helium for various paywall actions, purchase completions, and more. Options to listen for these events include:
...
If the budget expires before the paywall is ready, a fallback paywall will show if available otherwise the loading state will hide and a [PaywallOpenFailed](/guides/helium-events) event will be dispatched.
