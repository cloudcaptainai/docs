...
Set custom user traits for targeting and analytics. Traits persist across app sessions.
...
// You can also pass a dictionary directly:
Helium.identify.setUserTraits(["hasOnboarded": true, "accountAge": 30])
```

<Note>
  Numeric values set as `Int` may return as `Double` after app restart due to JSON serialization. Use `as? Double` when reading numeric traits with `getUserTraits()`.
</Note>
