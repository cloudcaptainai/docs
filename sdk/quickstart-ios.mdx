...
In Xcode, go to **Add Package Dependencies**, and copy in `https://github.com/cloudcaptainai/helium-swift.git`
...
If you are using RevenueCat to manage purchases, then we recommended you also add **HeliumRevenueCat** to your target so that you can use our RevenueCatDelegate referenced below. Otherwise use **None** for this row.
...
<Note>
  We highly recommend using SPM, but if your project is heavily reliant on Cocoapods, it might make sense to use our Cocoapod instead.
</Note>
...
To integrate Helium paywalls, create a subclass of `HeliumPaywallDelegate` or use one of our pre-built delegates from the next section. This class is responsible for handling the purchase logic for your paywalls.
...
### Making StoreKit purchases

<Note>
  If you're using a custom delegate instead of the default StoreKit 2 delegate, please use `Product.heliumPurchase()` instead of Product.purchase(). This will automatically set attribution information for revenue tracking. Our default StoreKit delegate does this by default.
</Note>
...
Helium.shared.initialize(
    // You can create or find your api key (a JWT key) by going to the dashboard and clicking on **Account Settings**.
    ...
    // [OPTIONAL] Mapping from trigger name to fallback paywall to show. If set, this value will override
    // the view passed into "fallback".
    fallbackPaywallPerTrigger: [String: any View]?,

    // [OPTIONAL] If set, a custom user id to use instead of Helium's. We'll use this id when forwarding to 
    // third party analytics services, so this can be used for attribution (e.g. an amplitude user id, or a custom user id from your own analytics service)
    customUserId: String? = nil,

    // [OPTIONAL] Pass in custom user traits to be used for targeting, personalization, and dynamic content.
...
Paywall Skipped - This event will fire when a paywall open gets _skipped_ for a trigger, due to a targeting or workflow configuration. When this event fires, we won't trigger opening the paywall modal - so this is a good place to handle any custom logic that should fire.
