...
<Accordion title="In-App Paywall Links">
  By default, web links (`http`/`https`) opened via a paywall's navigate action open in-app in a Safari page sheet over the paywall. To open them in the external browser instead:

  ```swift
  Helium.config.openPaywallLinksInApp = false
  ```
