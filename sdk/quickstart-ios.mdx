...
<ResponseField name="Helium.shared.presentPaywall" type="method">
  <Expandable title="parameters">
    ...
    <ResponseField name="onEntitled" type="((PaywallEntitledEvent) -> Void)?">
      _(Optional)_ A handler for when the user becomes entitled to a product in the paywall — through a purchase, a restore, an existing entitlement, or because the paywall was skipped for an already-entitled user.

      The closure receives a `PaywallEntitledEvent` telling you how entitlement occurred:

      - `.purchased(PurchaseSucceededEvent)` — a new purchase completed
      - `.restored(PurchaseRestoredEvent)` — entitlement surfaced via restore
      - `.alreadyEntitled(PurchaseAlreadyEntitledEvent)` — a purchase attempt resolved to an existing entitlement
      - `.skipped(PaywallSkippedEvent)` — paywall not shown because the user was already entitled (only when `PaywallPresentationConfig.dontShowIfAlreadyEntitled` is `true`)

      Accessors on the event:

      - `event: HeliumEvent` — the underlying SDK event, available for every case
      - `productId: String?` — the product id for `.purchased`, `.restored`, and `.alreadyEntitled`; `nil` for `.skipped`

      `.purchased`, `.restored`, and `.alreadyEntitled` are delivered when the paywall closes; `.skipped` fires immediately when the paywall is not shown.
...
Helium.shared.presentPaywall(
    ...
    onEntitled: { entitledEvent in
        switch entitledEvent {
        case .purchased:
            // new purchase completed
            break
        case .restored:
            // entitlement surfaced via restore
            break
        case .alreadyEntitled:
            // purchase attempt resolved to an existing entitlement
            break
        case .skipped:
            // paywall not shown because user was already entitled
            break
        }
    }
) { paywallNotShownReason in
...
<Accordion title="In-App Paywall Links">
