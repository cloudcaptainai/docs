...
<ResponseField name="Helium.shared.presentPaywall" type="method">
  <Expandable title="parameters">
    ...
    <ResponseField name="onEntitled" type="((PaywallEntitledEvent) -> Void)?">
      _(Optional)_ A handler for when user is entitled to a product in the paywall, via purchase or existing entitlement. Receives a `PaywallEntitledEvent` identifying how the user became entitled:
      ...
      The event's `productId` gives the product involved (`nil` for `.skipped`), and its `event` property exposes the underlying SDK event. Existing no-argument closures keep working (they map to a deprecated overload).
    </ResponseField>
...
Helium.shared.presentPaywall(
    ...
    onEntitled: { entitledEvent in
        ...
        // entitledEvent.productId — product involved (nil for .skipped)
    }
) { paywallNotShownReason in
    // handle paywall not shown
}
