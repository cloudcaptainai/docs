...
//   with a fallback bundle (recommended - see Fallbacks and Loading Budgets section)
...
Subclass one of the provided `HeliumPaywallDelegate` (see **Purchase Handling** section above) and override `onPaywallEvent`:
...
See the next section for details on PaywallEventHandlers.
...
You can show a paywall in one of 3 ways:
...
<ResponseField name="Helium.shared.presentUpsell" type="method">
  <Expandable title="parameters">
    ...
    <ResponseField name="dontShowIfAlreadyEntitled" type="Bool" default="false">
      _(Optional)_ If `true`, the paywall will not show if the user already has entitlements for any product in the paywall
    </ResponseField>
  </Expandable>
</ResponseField>
