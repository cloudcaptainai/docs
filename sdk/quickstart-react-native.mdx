...
<ResponseField name="PresentUpsellParams" type="type">
  <Expandable title="properties">
    ...
    <ResponseField name="onEntitled" type="((event?: PaywallEntitledEvent) => void)?">
      _(Optional)_ Called upon purchase success or purchase restore. If `dontShowIfAlreadyEntitled` is true, this handler will also be called when the paywall is not shown to users who already have entitlement for a product in the paywall. The handler receives an optional entitling event — see [PaywallEntitledEvent](/guides/helium-events#paywallentitledevent) for its shape and possible `type` values.
    </ResponseField>
