...
<Info>
  Both SDKs support **iOS and Android**. The Older Expo / Bare SDK requires React Native 0.71.7+, iOS 15+, and Android minSdk 24.
...
<Note>
  If you haven't already, make sure to [install](https://www.revenuecat.com/docs/getting-started/installation/expo) RevenueCat (for non-Expo see [here](https://www.revenuecat.com/docs/getting-started/installation/reactnative)). The Older Expo / Bare SDK requires `react-native-purchases` 8.0.0+.
...
### Web Checkout (Stripe & Paddle)

Sell web-based subscriptions directly from your paywalls. Enable it **before** `initialize()` and forward incoming deep links so the SDK can react to checkout redirects:

```tsx
import { enableExternalWebCheckout, heliumHandleURL } from 'expo-helium';
...
// forward returning deep links (requires the URL scheme registered in your app)
Linking.addEventListener('url', ({ url }) => heliumHandleURL(url));
```

Set a custom user id before showing web checkout paywalls. Full setup — products, webhook, and the hosted web paywall — is covered in the [Paddle onboarding guide](/guides/paddle-onboarding-guide). Web checkout currently completes on iOS; Android support is coming soon.
...
<Accordion title="Checking SDK State">
  ```typescript
