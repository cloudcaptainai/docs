...
Helium is an upsell creation and optimization tool for mobile apps. We take your existing paywalls (or help you create new ones!) 
...
- We'll take in your existing paywalls (or help you migrate them from 3rd party services), and then create handcrafted paywall + upsell screens using best practices we've learned from 10 years of growth product experience @ Uber, Meta, and Apple.
...
- About this package
    ...
    **helium-swift** is our open source SDK for iOS, available via SPM. It contains general initialization code that hooks up to our backend, and the view modifier logic.
...
public protocol HeliumPaywallDelegate: AnyObject {
    ...
    // [OPTIONAL] - Return a dictionary of custom variable values to be used in the paywall.
    // This method should return a dictionary of key-value pairs, where the key is the name of the variable and the value is the value of the variable.
    // These variables can be used to customize the paywall for each user.
...
We schedule it on a background thread, so you don't have to worry about it blocking your app's launch time.
